...
React + TanStack Start 기반 UI. Cloudflare 자동 배포 워크플로우는 이번 PR 범위에서 제외하되, Cloudflare 빌드 환경의 `wrangler versions upload`가 정적 산출물만 업로드할 수 있도록 assets-only 설정을 둡니다.
...
- `wrangler.jsonc`: Cloudflare 빌드 환경용 정적 assets-only 업로드 설정. Worker 엔트리/자동 배포 워크플로우는 포함하지 않음
