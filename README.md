...
React + TanStack Start 기반 UI. Cloudflare 빌드 환경의 `wrangler versions upload`가 실제 Worker 엔트리와 정적 산출물을 함께 업로드하도록 설정합니다.
...
3. WP 파일 트리 업로드 (단일 Git Trees API 커밋): `wp-admin/*`, `wp-content/themes/twentytwentyfour/*`, `wp-content/plugins/hello.php`, `wp-content/database/wordpress.db` (시드된 SQLite, base64), `worker/index.js` (사용자 Worker 코드 — nginx + WP 라우터). Cloudflare 자동 배포 파일은 이번 PR에서 생성하지 않음
...
5. Cloudflare 자동 배포는 이번 PR 범위에서 제외. 추후 사용자가 명시적으로 요청하면 수동 배포 안내 또는 별도 PR로 추가
...
- `src/worker-template/index.js`: 사용자 Worker 템플릿 (자동 배포 워크플로우 제외)
- `src/cloudflare-worker.ts`: 배포된 Worker에서 Vite `dist` assets를 서빙하고 `/health`, provision-log SSE를 처리
- `wrangler.jsonc`: Cloudflare 빌드 환경용 Worker + assets 업로드 설정
